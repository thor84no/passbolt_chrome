...
var listen = function (worker) {
  ...
  worker.port.on('passbolt.file.upload', function (token) {
    fileController.uploadFile().then(
      function success(content){
        worker.port.emit('passbolt.file.upload.complete', token, 'SUCCESS', content);
      },
      function failure() {
        worker.port.emit('passbolt.file.upload.complete', token, 'ERROR', true);
      }
    );
  });
